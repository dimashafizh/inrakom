...
$dataKerusakan = $kerusakan->getAllDatakerusakan();
$radioList = $kerusakan->getRadioList();

    // Untuk update informasi kerusakan
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_kerusakan'])) {
        ...
        if ($kerusakan->updateKerusakan($id_peminjaman, $id_radio, $nomor_sn, $catatan, $tanggal_baru)) {
            $_SESSION['alert'] = ['type' => 'primary', 'message' => 'Berhasil memperbarui data kerusakan'];
            header("Location: kerusakan.php");
            exit;
        } else {
            $_SESSION['alert'] = ['type' => 'danger', 'message' => 'Gagal memperbarui data kerusakan'];
        }
    }

// Untuk hapus data kerusakan
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['hapus_kerusakan'])) {
    $id_peminjaman = $_POST['id_peminjaman'];
    $hapus = $kerusakan->hapusKerusakan($id_peminjaman);

    if ($hapus === true) {
        $_SESSION['alert'] = ['type' => 'dark', 'message' => 'Data kerusakan telah dihapus!'];
        header("Location: kerusakan.php");
        exit;
    } elseif ($hapus === 'used') {
        $_SESSION['alert'] = ['type' => 'warning', 'message' => 'Data kerusakan tidak bisa dihapus karena sedang digunakan dalam tabel pendataan'];
        header("Location: kerusakan.php");
        exit;
    } else {
        $_SESSION['alert'] = ['type' => 'danger', 'message' => 'Gagal menghapus data kerusakan'];
    }
}
...
if (isset($_POST['filter_tanggal'])) {
    ...
    $dataKerusakan = $kerusakan->getDataKerusakanByTanggal($tanggal_mulai, $tanggal_selesai);
} else {
    $dataKerusakan = $kerusakan->getDataKerusakanByTanggal();
}
...
<html lang="en">
    <head>
        ...
        <title>Kerusakan | INRAKOM BINSUA</title>
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid">
                        <h1 class="mt-4"><i class="fa-solid fa-screwdriver-wrench"></i> Data Kerusakan Radio Komunikasi</h1>
                        
                        <div class="card mb-4">
                            <div class="card-header">
                                <a href="exportdata-kerusakan.php" class="btn btn-secondary">Export Data</a>
                                <br>
                                <div class="row mt-4">
                                    <div class="mb-3">
                                ...
                                <table id="datatablesSimple">
                                    ...
                                    <?php
                                        ...
                                        foreach ($dataKerusakan as $row) {
                                            ...
                                            // Modal Edit untuk data kerusakan
                                            echo "
                                            <div class='modal fade' id='edit-{$row['id_peminjaman']}'>
                                                <div class='modal-dialog'>
                                                    <div class='modal-content'>
                                                        <div class='modal-header'>
                                                            <h4 class='modal-title'><i class='fa-solid fa-pen-to-square'></i> Edit Data Kerusakan</h4>
                                            ...
                                            echo "              </select>
                                                            ...
                                                            <div class='modal-footer'>
                                                                <button type='submit' name='update_kerusakan' class='btn btn-primary'>Update</button>
                                                            </div>
                                                        </form>
                                                    </div>
                                                </div>
                                            ...
                                            // Modal untuk hapus data kerusakan
                                            echo "
                                            <div class='modal fade' id='hapus-{$row['id_peminjaman']}'>
                                                <div class='modal-dialog'>
                                                    <div class='modal-content'>
                                                        <div class='modal-header'>
                                                            <h4 class='modal-title'><i class='fa-solid fa-triangle-exclamation'></i> Konfirmasi Hapus Data Kerusakan</h4>
                                                            <button type='button' class='btn-close' data-bs-dismiss='modal'></button>
                                                        </div>
                                                        ...
                                                        <div class='modal-footer'>
                                                            <form method='POST' action=''>
                                                                <input type='hidden' name='id_peminjaman' value='{$row['id_peminjaman']}'>
                                                                <button type='submit' name='hapus_kerusakan' class='btn btn-danger'>Ya, hapus!</button>
                                                            </form>
                                                        </div>
                                                    </div>
                                                </div>
